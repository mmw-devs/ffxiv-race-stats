...
当前 `.pi/skills/` 下存在 6 个 Skill 文件。每次 data.json 修改都需触发多条 Skill 链，Agent Loop 读取开销随 Skill 数量线性增长。本报告按 YAGNI 原则逐项审查，识别可精简的部分，为后续 #43（All-in-One 运营 Skill）设计提供决策依据。
...
- **🟡 add-news：** 仅在首杀通关、重大事件节点触发。一场赛事预计 5-10 次。
...
- 为 #49（Agentic RAG 工具检索）提供结构化 action 空间
...
| 优先级 | 行动 | 依赖 |
|:---:|------|------|
| P0 | 三合一声明式映射表设计与实现 | #43 设计完成 + 负责人确认方向 |
