...
| 第二阶段重构基于第一阶段产出，每步 PR 可独立 review / 合入 | ✅ PR-1 + PR-1-cleanup + PR-2 + PR-3 + PR-4 已完成 | N4 §3.1.3 + §3.1.4 + §3.1.5 + §3.1.6 + §3.1.7 |
| PI Agent 不可靠点有清晰兜底策略（无论是集中还是分散） | ✅ 已完成 | N2 §8 + 本文档 §7 |
| SKILL.md 注入问题得到解决，PI Agent 真正看到协议 | ✅ PR-1 + PR-2 + PR-3 + PR-4 已完成 | N4 §3.1.3 + §3.1.5 + §3.1.6 + §3.1.7 + lark-bot-protocol/SKILL.md §2 §3 §4 §6 |
| PR#163 中发现的边界 bug 都有针对性回归测试 | ✅ PR-1 + PR-2 + PR-3 + PR-4 已完成边界回归 | N4 §9 + PR-1/PR-2/PR-3/PR-4 新增测试 |
| 现有 195 测试 + 重构期间新增测试全过 | ✅ PR-1 + PR-2 + PR-3 + PR-4 验证通过（250 测试） | N4 §9 + 各 PR 单测 |
