...
5. validator 只接受已确认 candidate；失败会恢复 baseline candidate，成功后才可展示已校验结果。
6. 工具只生成 candidate artifact，不会修改 `public/data.json`，也不得调用旧 `update-team`、`content-pr` Skill。
