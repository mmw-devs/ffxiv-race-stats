...
test("apply 后 content-pr 可以识别已应用 workspace",async t=>{const e=await fixture();t.after(e.cleanup);await e.applier.apply(e.state.taskId);const calls=[];const adapter=new ContentPrAdapter({taskStore:e.store,workspaceRoot:e.workspace,run:(bin,args)=>{calls.push([bin,args]);if(bin==="gh")return "https://example.test/pr/1";if(bin==="git"&&args[0]==="branch")return "main\n";if(bin==="git"&&args[0]==="status")return " M public/data.json\n";return "";}});const result=await adapter.submit(e.state.taskId);assert.equal(result.state.lifecycle.state,"PR_CREATED");assert.equal(calls.filter(([bin])=>bin==="gh").length,1);});
