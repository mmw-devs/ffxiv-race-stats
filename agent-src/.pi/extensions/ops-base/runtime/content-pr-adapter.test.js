...
const { WorkspaceCandidateApplier } = require("./workspace-candidate-applier.js");
...
test("workspace changes 与已验证记录不一致时拒绝",async(t)=>{const e=await fixture();t.after(e.cleanup);const bad=structuredClone(changed);bad.teams[0].bossHP=35;await fs.writeFile(path.join(e.adapter.workspaceRoot,"public/data.json"),JSON.stringify(bad));await assert.rejects(e.adapter.submit(e.state.taskId),(x)=>x.code==="WORKSPACE_DIFF_MISMATCH");assert.equal(e.commands.some(([file,args]) => file === "git" && args[0] === "checkout"),false);});
